...
const GeminiDice = (() => {
  ...
  const s = {
    outer:
      "background: white; border: 1px solid black; padding: 5px 3px; color: black; border-radius: 15px;",
    failsum:
      "display: table-cell; text-align: center; vertical-align: center; line-height: 80px; font-size: 3em; background: #b30000; height: 80px; width: 80px; border-radius: 40px; border: 1px solid black; color: white; font-weight: bold;",
  ...
  const who = (msg) => {
    return (getObj("player", msg.playerid) || { get: () => "API" }).get(
      "_displayname"
    );
  };
  ...
  const initResult = (msg, type = ROLL_TYPE_STANDARD) => {
    ...
    if (
      msg.inlinerolls[0].results.rolls.length >= 2 &&
      msg.inlinerolls[0].results.rolls[1].hasOwnProperty("expr")
    ) {
      if (
        /^\+\([0-9]*\+[0-9]*\)/.test(msg.inlinerolls[0].results.rolls[1].expr)
      ) {
        let pipParts = msg.inlinerolls[0].results.rolls[1].expr
          .slice(2, -1)
          .split("+");

        pips = parseInt(pipParts[0], 10) + parseInt(pipParts[1], 10);
      } else if (
        /^(\+|\-)[0-9]*/.test(msg.inlinerolls[0].results.rolls[1].expr)
      ) {
  ...
  const wildDie = (result) => {
    if (result.wildDie === ROLL_CRIT_FAILURE) {
      if (result.type === ROLL_TYPE_STANDARD) {
        ...
        result.sumFail =
          Object.keys(critFailDice).reduce(
            (m, k) => m + parseInt(k, 10) * critFailDice[k],
            0
          ) + result.pips;
      } else if (result.type === ROLL_TYPE_SUCCESS) {
        let successes = [...result.dice, ...result.bonusDice].filter(
          (d) => d >= SUCCESS_THRESHOLD
        ).length;
  ...
  const sendResultMessage = (result, w, msg) => {
    ...
    if (/^template\b\s/.test(parts(msg)[1] || "")) {
      let template = `&{template:${msg.rolltemplate}}${parts(msg)[1].replace(
        /^template\s+/i,
        ""
      )}`;
  ...
  const getPips = (result) => {
    ...
    return `<div style="display: block ; text-align: center; font-size: 0.85em ; color: #595959; padding-top: 5px">Total has had ${sign}${
      result.pips
    } pip${result.pips > 1 ? "s" : ""} ${
  ...
  const getTotal = (result) => {
    ...
    if (result.type === ROLL_TYPE_STANDARD) {
      sum =
        [...result.dice, ...result.bonusDice, ...[result.wildDie]].reduce(
          (a, b) => a + b,
          0
        ) + result.pips;
    } else if (result.type === ROLL_TYPE_SUCCESS) {
      sum =
        [...result.dice, ...result.bonusDice].filter(
          (d) => d >= SUCCESS_THRESHOLD
        ).length + (result.wildDie >= SUCCESS_THRESHOLD ? 1 : 0);
    } else if (
      result.type === ROLL_TYPE_LUCK ||
      result.type === ROLL_TYPE_UNLUCK
    ) {
      const threshold =
        result.type === ROLL_TYPE_LUCK ? LUCK_THRESHOLD : UNLUCK_THRESHOLD;

      sum = [...result.dice, ...result.bonusDice, ...[result.wildDie]].filter(
        (d) => d === threshold
      ).length;
    }
    ...
    return (
      ...
      (isFailed ? f.failsum(result.sumFail < 0 ? 0 : result.sumFail) : "") +
      '<div style="display: table-cell; margin: auto; width: 50%; text-align: center; vertical-align: center; line-height: 80px; font-size: 3em; background: ' +
      (isFailed ? "#f09f1f" : "#25c21d") +
      '; height: 80px; width: 80px; border-radius: 40px; border: 1px solid black; color: white; font-weight: bold;">' +
      sum +
      "</div></div>" +
